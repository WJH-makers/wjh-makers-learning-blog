...
import { SEARCH_SEASONS, SEARCH_SERIES_META, searchPublishedEpisodes, searchAllEpisodes } from "@/lib/series-search";
...
export const metadata: Metadata = {
  title: `${SEARCH_SERIES_META.title} · ${SEARCH_SERIES_META.alias}`,
  description: SEARCH_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/search-engine` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: searchPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${SEARCH_SERIES_META.title} · ${SEARCH_SERIES_META.alias}`,
    description: SEARCH_SERIES_META.tagline,
    url: `${siteUrl()}/search-engine`,
    type: "website",
  },
};

export default function SearchSeriesPage() {
  const total = searchAllEpisodes().length;
  const done = searchPublishedEpisodes().length;
  const progressSeasons = SEARCH_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{SEARCH_SERIES_META.title}</h1>
          <p className="hero-text">{SEARCH_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${searchPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{SEARCH_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={SEARCH_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={SEARCH_SEASONS} storageKey={SEARCH_SERIES_META.storageKey} />
