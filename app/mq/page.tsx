...
import { MQ_SEASONS, MQ_SERIES_META, mqPublishedEpisodes, mqAllEpisodes } from "@/lib/series-mq";
...
export const metadata: Metadata = {
  title: `${MQ_SERIES_META.title} · ${MQ_SERIES_META.alias}`,
  description: MQ_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/mq` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: mqPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${MQ_SERIES_META.title} · ${MQ_SERIES_META.alias}`,
    description: MQ_SERIES_META.tagline,
...
export default function MqSeriesPage() {
  const total = mqAllEpisodes().length;
  const done = mqPublishedEpisodes().length;
  const progressSeasons = MQ_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: mqPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{MQ_SERIES_META.title}</h1>
          <p className="hero-text">{MQ_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${mqPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{MQ_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={MQ_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={MQ_SEASONS} storageKey={MQ_SERIES_META.storageKey} />

      {MQ_SEASONS.map((season) => (
