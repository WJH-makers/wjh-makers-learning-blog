...
import { CLOUD_SEASONS, CLOUD_SERIES_META, cloudPublishedEpisodes, cloudAllEpisodes } from "@/lib/series-cloud";
...
export const metadata: Metadata = {
  title: `${CLOUD_SERIES_META.title} · ${CLOUD_SERIES_META.alias}`,
  description: CLOUD_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/cloud` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: cloudPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${CLOUD_SERIES_META.title} · ${CLOUD_SERIES_META.alias}`,
    description: CLOUD_SERIES_META.tagline,
...
export default function CloudSeriesPage() {
  const total = cloudAllEpisodes().length;
  const done = cloudPublishedEpisodes().length;
  const progressSeasons = CLOUD_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: cloudPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{CLOUD_SERIES_META.title}</h1>
          <p className="hero-text">{CLOUD_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${cloudPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{CLOUD_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={CLOUD_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={CLOUD_SEASONS} storageKey={CLOUD_SERIES_META.storageKey} />

      {CLOUD_SEASONS.map((season) => (
