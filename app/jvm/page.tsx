...
import { JVM_SEASONS, JVM_SERIES_META, jvmPublishedEpisodes, jvmAllEpisodes } from "@/lib/series-jvm";
...
export const metadata: Metadata = {
  title: `${JVM_SERIES_META.title} · ${JVM_SERIES_META.alias}`,
  description: JVM_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/jvm` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: jvmPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${JVM_SERIES_META.title} · ${JVM_SERIES_META.alias}`,
    description: JVM_SERIES_META.tagline,
...
export default function JvmSeriesPage() {
  ...
  const seriesJsonLd = {
    ...
    hasPart: jvmPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{JVM_SERIES_META.title}</h1>
          <p className="hero-text">{JVM_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${jvmPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{JVM_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={JVM_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={JVM_SEASONS} storageKey={JVM_SERIES_META.storageKey} />

      {JVM_SEASONS.map((season) => (
