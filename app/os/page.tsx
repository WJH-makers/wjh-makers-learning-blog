...
import { OS_SEASONS, OS_SERIES_META, osPublishedEpisodes, osAllEpisodes } from "@/lib/series-os";
...
export const metadata: Metadata = {
  title: `${OS_SERIES_META.title} · ${OS_SERIES_META.alias}`,
  description: OS_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/os` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: osPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${OS_SERIES_META.title} · ${OS_SERIES_META.alias}`,
    description: OS_SERIES_META.tagline,
...
export default function OsSeriesPage() {
  const total = osAllEpisodes().length;
  const done = osPublishedEpisodes().length;
  const progressSeasons = OS_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: osPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{OS_SERIES_META.title}</h1>
          <p className="hero-text">{OS_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${osPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{OS_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={OS_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={OS_SEASONS} storageKey={OS_SERIES_META.storageKey} />

      {OS_SEASONS.map((season) => (
