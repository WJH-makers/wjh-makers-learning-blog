...
import { SRC_SEASONS, SRC_SERIES_META, srcPublishedEpisodes, srcAllEpisodes } from "@/lib/series-src";
...
export const metadata: Metadata = {
  title: `${SRC_SERIES_META.title} · ${SRC_SERIES_META.alias}`,
  description: SRC_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/src` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: srcPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${SRC_SERIES_META.title} · ${SRC_SERIES_META.alias}`,
    description: SRC_SERIES_META.tagline,
...
export default function SrcSeriesPage() {
  const total = srcAllEpisodes().length;
  const done = srcPublishedEpisodes().length;
  const progressSeasons = SRC_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: srcPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{SRC_SERIES_META.title}</h1>
          <p className="hero-text">{SRC_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${srcPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{SRC_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={SRC_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={SRC_SEASONS} storageKey={SRC_SERIES_META.storageKey} />

      {SRC_SEASONS.map((season) => (
