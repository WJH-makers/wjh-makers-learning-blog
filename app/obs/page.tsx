...
import { OBS_SEASONS, OBS_SERIES_META, obsPublishedEpisodes, obsAllEpisodes } from "@/lib/series-obs";
...
export const metadata: Metadata = {
  title: `${OBS_SERIES_META.title} · ${OBS_SERIES_META.alias}`,
  description: OBS_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/obs` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: obsPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${OBS_SERIES_META.title} · ${OBS_SERIES_META.alias}`,
    description: OBS_SERIES_META.tagline,
...
export default function ObsSeriesPage() {
  const total = obsAllEpisodes().length;
  const done = obsPublishedEpisodes().length;
  const progressSeasons = OBS_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: obsPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{OBS_SERIES_META.title}</h1>
          <p className="hero-text">{OBS_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${obsPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{OBS_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={OBS_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={OBS_SEASONS} storageKey={OBS_SERIES_META.storageKey} />

      {OBS_SEASONS.map((season) => (
