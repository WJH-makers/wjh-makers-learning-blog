...
import { BIGDATA_SEASONS, BIGDATA_SERIES_META, bigdataPublishedEpisodes, bigdataAllEpisodes } from "@/lib/series-bigdata";
...
export const metadata: Metadata = {
  title: `${BIGDATA_SERIES_META.title} · ${BIGDATA_SERIES_META.alias}`,
  description: BIGDATA_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/bigdata` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: bigdataPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${BIGDATA_SERIES_META.title} · ${BIGDATA_SERIES_META.alias}`,
    description: BIGDATA_SERIES_META.tagline,
...
export default function BigdataSeriesPage() {
  const total = bigdataAllEpisodes().length;
  const done = bigdataPublishedEpisodes().length;
  const progressSeasons = BIGDATA_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    name: BIGDATA_SERIES_META.title,
    url: `${siteUrl()}/bigdata`,
    description: BIGDATA_SERIES_META.tagline,
    inLanguage: "zh-CN",
    author: {
    ...
    hasPart: bigdataPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{BIGDATA_SERIES_META.title}</h1>
          <p className="hero-text">{BIGDATA_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${bigdataPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{BIGDATA_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={BIGDATA_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={BIGDATA_SEASONS} storageKey={BIGDATA_SERIES_META.storageKey} />

      {BIGDATA_SEASONS.map((season) => (
