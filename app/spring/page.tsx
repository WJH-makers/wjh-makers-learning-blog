...
import { SPRING_SEASONS, SPRING_SERIES_META, springPublishedEpisodes, springAllEpisodes } from "@/lib/series-spring";
...
export const metadata: Metadata = {
  title: `${SPRING_SERIES_META.title} · ${SPRING_SERIES_META.alias}`,
  description: SPRING_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/spring` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: springPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${SPRING_SERIES_META.title} · ${SPRING_SERIES_META.alias}`,
    description: SPRING_SERIES_META.tagline,
...
export default function SpringSeriesPage() {
  const total = springAllEpisodes().length;
  const done = springPublishedEpisodes().length;
  const progressSeasons = SPRING_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: springPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{SPRING_SERIES_META.title}</h1>
          <p className="hero-text">{SPRING_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${springPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{SPRING_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={SPRING_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={SPRING_SEASONS} storageKey={SPRING_SERIES_META.storageKey} />

      {SPRING_SEASONS.map((season) => (
