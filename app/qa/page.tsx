...
import { QA_SEASONS, QA_SERIES_META, qaPublishedEpisodes, qaAllEpisodes } from "@/lib/series-qa";
...
export const metadata: Metadata = {
  title: `${QA_SERIES_META.title} · ${QA_SERIES_META.alias}`,
  description: QA_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/qa` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: qaPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${QA_SERIES_META.title} · ${QA_SERIES_META.alias}`,
    description: QA_SERIES_META.tagline,
...
export default function QaSeriesPage() {
  const total = qaAllEpisodes().length;
  const done = qaPublishedEpisodes().length;
  const progressSeasons = QA_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: qaPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{QA_SERIES_META.title}</h1>
          <p className="hero-text">{QA_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${qaPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{QA_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={QA_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={QA_SEASONS} storageKey={QA_SERIES_META.storageKey} />

      {QA_SEASONS.map((season) => (
