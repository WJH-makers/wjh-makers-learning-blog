...
import { GITADV_SEASONS, GITADV_SERIES_META, gitadvPublishedEpisodes, gitadvAllEpisodes } from "@/lib/series-gitadv";
...
export const metadata: Metadata = {
  title: `${GITADV_SERIES_META.title} · ${GITADV_SERIES_META.alias}`,
  description: GITADV_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/gitadv` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: gitadvPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${GITADV_SERIES_META.title} · ${GITADV_SERIES_META.alias}`,
    description: GITADV_SERIES_META.tagline,
...
export default function GitadvSeriesPage() {
  const total = gitadvAllEpisodes().length;
  const done = gitadvPublishedEpisodes().length;
  const progressSeasons = GITADV_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: gitadvPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{GITADV_SERIES_META.title}</h1>
          <p className="hero-text">{GITADV_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${gitadvPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{GITADV_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={GITADV_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={GITADV_SEASONS} storageKey={GITADV_SERIES_META.storageKey} />

      {GITADV_SEASONS.map((season) => (
