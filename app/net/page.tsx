...
import { NET_SEASONS, NET_SERIES_META, netPublishedEpisodes, netAllEpisodes } from "@/lib/series-net";
...
export default function NetSeriesPage() {
  const total = netAllEpisodes().length;
  const done = netPublishedEpisodes().length;
  const progressSeasons = NET_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: netPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{NET_SERIES_META.title}</h1>
          <p className="hero-text">{NET_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${netPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{NET_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={NET_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={NET_SEASONS} storageKey={NET_SERIES_META.storageKey} />

      {NET_SEASONS.map((season) => (
