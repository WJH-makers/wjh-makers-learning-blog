...
import { MICRO_SEASONS, MICRO_SERIES_META, microPublishedEpisodes, microAllEpisodes } from "@/lib/series-micro";
...
export default function MicroSeriesPage() {
  const total = microAllEpisodes().length;
  const done = microPublishedEpisodes().length;
  const progressSeasons = MICRO_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: microPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{MICRO_SERIES_META.title}</h1>
          <p className="hero-text">{MICRO_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${microPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{MICRO_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={MICRO_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={MICRO_SEASONS} storageKey={MICRO_SERIES_META.storageKey} />

      {MICRO_SEASONS.map((season) => (
