...
import { PERF_SEASONS, PERF_SERIES_META, perfPublishedEpisodes, perfAllEpisodes } from "@/lib/series-perf";
...
export const metadata: Metadata = {
  title: `${PERF_SERIES_META.title} · ${PERF_SERIES_META.alias}`,
  description: PERF_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/perf` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: perfPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${PERF_SERIES_META.title} · ${PERF_SERIES_META.alias}`,
    description: PERF_SERIES_META.tagline,
...
export default function PerfSeriesPage() {
  const total = perfAllEpisodes().length;
  const done = perfPublishedEpisodes().length;
  const progressSeasons = PERF_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: perfPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{PERF_SERIES_META.title}</h1>
          <p className="hero-text">{PERF_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${perfPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{PERF_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={PERF_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={PERF_SEASONS} storageKey={PERF_SERIES_META.storageKey} />

      {PERF_SEASONS.map((season) => (
