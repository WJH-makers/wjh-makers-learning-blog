...
import { BUILD_SEASONS, BUILD_SERIES_META, buildPublishedEpisodes, buildAllEpisodes } from "@/lib/series-build";
...
export const metadata: Metadata = {
  title: `${BUILD_SERIES_META.title} · ${BUILD_SERIES_META.alias}`,
  description: BUILD_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/build` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: buildPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${BUILD_SERIES_META.title} · ${BUILD_SERIES_META.alias}`,
    description: BUILD_SERIES_META.tagline,
...
export default function BuildSeriesPage() {
  const total = buildAllEpisodes().length;
  const done = buildPublishedEpisodes().length;
  const progressSeasons = BUILD_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: buildPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{BUILD_SERIES_META.title}</h1>
          <p className="hero-text">{BUILD_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${buildPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{BUILD_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={BUILD_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={BUILD_SEASONS} storageKey={BUILD_SERIES_META.storageKey} />

      {BUILD_SEASONS.map((season) => (
