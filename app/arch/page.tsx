...
import { ARCH_SEASONS, ARCH_SERIES_META, archPublishedEpisodes, archAllEpisodes } from "@/lib/series-arch";
...
export const metadata: Metadata = {
  title: `${ARCH_SERIES_META.title} · ${ARCH_SERIES_META.alias}`,
  description: ARCH_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/arch` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: archPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${ARCH_SERIES_META.title} · ${ARCH_SERIES_META.alias}`,
    description: ARCH_SERIES_META.tagline,
...
export default function ArchSeriesPage() {
  const total = archAllEpisodes().length;
  const done = archPublishedEpisodes().length;
  const progressSeasons = ARCH_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: archPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{ARCH_SERIES_META.title}</h1>
          <p className="hero-text">{ARCH_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${archPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{ARCH_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={ARCH_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={ARCH_SEASONS} storageKey={ARCH_SERIES_META.storageKey} />

      {ARCH_SEASONS.map((season) => (
