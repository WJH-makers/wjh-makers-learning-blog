...
import { REACTIVE_SEASONS, REACTIVE_SERIES_META, reactivePublishedEpisodes, reactiveAllEpisodes } from "@/lib/series-reactive";
...
export const metadata: Metadata = {
  title: `${REACTIVE_SERIES_META.title} · ${REACTIVE_SERIES_META.alias}`,
  description: REACTIVE_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/reactive` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: reactivePublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${REACTIVE_SERIES_META.title} · ${REACTIVE_SERIES_META.alias}`,
    description: REACTIVE_SERIES_META.tagline,
    url: `${siteUrl()}/reactive`,
    type: "website",
  },
};

export default function ReactiveSeriesPage() {
  const total = reactiveAllEpisodes().length;
  const done = reactivePublishedEpisodes().length;
  const progressSeasons = REACTIVE_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    name: REACTIVE_SERIES_META.title,
    url: `${siteUrl()}/reactive`,
    description: REACTIVE_SERIES_META.tagline,
    inLanguage: "zh-CN",
    author: {
    ...
    hasPart: reactivePublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{REACTIVE_SERIES_META.title}</h1>
          <p className="hero-text">{REACTIVE_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${reactivePublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{REACTIVE_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={REACTIVE_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={REACTIVE_SEASONS} storageKey={REACTIVE_SERIES_META.storageKey} />

      {REACTIVE_SEASONS.map((season) => (
