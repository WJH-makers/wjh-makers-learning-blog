...
import { AI_SEASONS, AI_SERIES_META, aiPublishedEpisodes, aiAllEpisodes } from "@/lib/series-ai";
...
export const metadata: Metadata = {
  title: `${AI_SERIES_META.title} · ${AI_SERIES_META.alias}`,
  description: AI_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/ai` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: aiPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${AI_SERIES_META.title} · ${AI_SERIES_META.alias}`,
    description: AI_SERIES_META.tagline,
...
export default function AiSeriesPage() {
  const total = aiAllEpisodes().length;
  const done = aiPublishedEpisodes().length;
  const progressSeasons = AI_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: aiPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{AI_SERIES_META.title}</h1>
          <p className="hero-text">{AI_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${aiPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{AI_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={AI_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={AI_SEASONS} storageKey={AI_SERIES_META.storageKey} />

      {AI_SEASONS.map((season) => (
