...
import { DIST_SEASONS, DIST_SERIES_META, distPublishedEpisodes, distAllEpisodes } from "@/lib/series-dist";
...
export const metadata: Metadata = {
  title: `${DIST_SERIES_META.title} · ${DIST_SERIES_META.alias}`,
  description: DIST_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/dist` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: distPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${DIST_SERIES_META.title} · ${DIST_SERIES_META.alias}`,
    description: DIST_SERIES_META.tagline,
...
export default function DistSeriesPage() {
  const total = distAllEpisodes().length;
  const done = distPublishedEpisodes().length;
  const progressSeasons = DIST_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: distPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{DIST_SERIES_META.title}</h1>
          <p className="hero-text">{DIST_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${distPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{DIST_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={DIST_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={DIST_SEASONS} storageKey={DIST_SERIES_META.storageKey} />

      {DIST_SEASONS.map((season) => (
