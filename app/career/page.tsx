...
import { CAREER_SEASONS, CAREER_SERIES_META, careerPublishedEpisodes, careerAllEpisodes } from "@/lib/series-career";
...
export const metadata: Metadata = {
  title: `${CAREER_SERIES_META.title} · ${CAREER_SERIES_META.alias}`,
  description: CAREER_SERIES_META.tagline,
  alternates: { canonical: `${siteUrl()}/career` },
  // 未开更的蓝图页对搜索引擎是薄内容;开更后自动恢复索引。
  robots: careerPublishedEpisodes().length === 0 ? { index: false, follow: true } : undefined,
  openGraph: {
    ...OG_BASE,
    title: `${CAREER_SERIES_META.title} · ${CAREER_SERIES_META.alias}`,
    description: CAREER_SERIES_META.tagline,
...
export default function CareerSeriesPage() {
  const total = careerAllEpisodes().length;
  const done = careerPublishedEpisodes().length;
  const progressSeasons = CAREER_SEASONS.map((s) => ({
    code: s.code,
    title: s.title,
  ...
  const seriesJsonLd = {
    ...
    hasPart: careerPublishedEpisodes().map((ep, i) => ({
      "@type": "BlogPosting",
      position: i + 1,
      name: ep.title,
    ...
    <div className="page-shell">
      ...
      <section className="hero">
        <div>
          ...
          <h1>{CAREER_SERIES_META.title}</h1>
          <p className="hero-text">{CAREER_SERIES_META.tagline}</p>
          <div className="hero-actions">
            {done > 0 && (
              <Link className="button primary" href={`/posts/${careerPublishedEpisodes()[0].slug}`}>
                从第一话开始 →
              </Link>
            )}
        ...
        <div className="hero-panel">
          ...
          <p className="muted">
            长期项目:{CAREER_SERIES_META.project}
          </p>
        </div>
      </section>

      {progressSeasons.length > 0 && (
        <JavaProgress seasons={progressSeasons} storageKey={CAREER_SERIES_META.storageKey} />
      )}
      ...
      <SeriesMap seasons={CAREER_SEASONS} storageKey={CAREER_SERIES_META.storageKey} />
