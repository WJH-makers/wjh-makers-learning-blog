...
豆豆:「InnoDB 默认的 REPEATABLE READ 下,**普通一致性读看到的是第一次一致性读建立的世界**,插入/更新却要面对『现在的世界』——中间别人提交了什么,后续普通 SELECT 通常看不见。」
...
> **豆豆锐评**:RR 下的快照读给你看的是「本事务第一次一致性读建立的世界」,不是「现在的世界」；`START TRANSACTION WITH CONSISTENT SNAPSHOT` 是显式在事务开始建立快照的例外。拿快照读当实时真相做决定,是事务类 Bug 的头号来源。
...
-- name 无索引:可能扫描并锁住大量(极端时几乎全部)索引记录,严重挤压并发;这不是字面意义的表锁
```
...
> 两条腿:**快照读靠 MVCC**——RR 下从第一次一致性读起复用同一快照,后来者提交的行对后续普通 SELECT 不可见;**当前读靠临键锁(Next-Key Lock=行锁+间隙锁)**——记录连同「缝隙」一起锁,不许往范围里插。追问点:先快照读再 `FOR UPDATE`,结果可能不同——不是 RR 破功,两种读本就看不同的世界。
...
5. RR 隔离级别下,普通一致性读的 ReadView 通常何时建立?
- A) 每条 SELECT 语句执行时新生成一个
- B) **第一次普通一致性读时生成**,之后同事务的普通一致性读共用
- C) 每次 UPDATE 时生成
- D) 由用户手动指定
...
- A) 行锁直接锁在数据行上
- B) 行锁加在**索引记录**上;若 WHERE 条件无索引,可能扫描并锁住大量索引记录
- C) 行锁只在主键索引上加
- D) 行锁不会产生死锁
...
10. RR 隔离级别防幻读靠的是?
- A) 只靠 MVCC 快照读
- B) RR 的快照读靠 MVCC(第一次一致性读后的快照使新提交行对普通 SELECT 不可见) + 当前读靠临键锁(记录锁 + 间隙锁,不许往范围里插)
- C) 只靠间隙锁
...
>
> 5-B。RR 下,**第一次普通一致性读**建立 ReadView,之后的普通一致性读共用；RC 下每条一致性读都有新快照。若要在事务开始就建立快照，可显式使用 `START TRANSACTION WITH CONSISTENT SNAPSHOT`。★举一反三:因为 RR 复用快照,所以「可重复读」;因为 RC 每次一致性读都刷新,所以能看到别人提交的更新(不可重复读)。
>
> 6-A。A 的第一次普通 SELECT 建立 ReadView,之后别人提交的数据对其后续普通 SELECT 不可见；但 INSERT 要检查当前唯一索引，发现已有记录就会 Duplicate。★举一反三:隔离性只保证「读得一致」,不保证「读后别人不动」。唯一约束才是防重写入的守门员。
>
> 7-B。锁加在索引记录上。`UPDATE … WHERE name='阿零'` 若 name 无索引,可能扫描并锁住大量、甚至几乎全部索引记录，严重损害并发；但它不是 InnoDB 的字面「表锁」。★举一反三:这解释了为什么 WHERE 条件列必须有索引——不仅为查询快,也为锁粒度合理。
>
...
>
> 10-B。RR 的快照读:第一次一致性读建立的 ReadView 使后来者提交的行对后续普通 SELECT 不可见→**读层面防幻读**。当前读:临键锁(Next-Key Lock=行锁+间隙锁)把记录和缝隙一起锁→**写层面防幻读**。★举一反三:仅靠 MVCC 无法防止当前读的幻读(你查不到但别人能插入),必须配合锁。
>
> **解答题**
>
> **Q1** 隐藏列:trx_id(事务 ID,记录谁改的)和 roll_pointer(指向上一个版本的 undo 指针)。undo 版本链:从当前最新版本顺着 roll_pointer 往回串,每版标注了 trx_id。ReadView:一致性读建立的快照,包含活跃事务名单——用于判断哪个版本对当前事务可见。三次 UPDATE 后的链:`[行当前:trx=300]→roll_ptr→[trx=250]→roll_ptr→[trx=100 初始]→roll_ptr=NULL`。★举一反三:ReadView 通过 trx_id 在版本链上从新到旧找到第一个「可见」的版本——这就是「时间机器」的运转方式。
>
> **Q2** RC:每条一致性读重新生成 ReadView → 别人提交的更新对后续 SELECT 可见 → 同一事务内两次读同一行可能读到不同值(不可重复读)。RR:第一次一致性读生成一张 ReadView,之后普通一致性读共用 → 可重复读。★举一反三:这个差异从代码看只是「ReadView 创建时机的不同」——原理越简单,区分越重要。
>
