...
**三色标记一分钟直觉**:并发收集 = 清洁队与顾客同场。对象染三色:白(没查)、灰(自己查了邻居没查完)、黑(查完)。若用户线程把白对象挂到**黑**对象名下、又剪断它与灰对象的旧连接,这个活对象就会被漏标误删。补救靠**屏障**:引用一变动就记账,收尾照账补扫。G1 走**写屏障**(SATB,赋值前记下旧引用快照);ZGC 走**读屏障**(读指针时顺手校正)——手段不同,目标一致:不漏活对象。
...
10. 选型决策表中,一个堆大小 8GB、延迟敏感的在线交易服务,首选是?
   - A) Serial
   - B) Parallel
   - C) G1 + 收紧 `MaxGCPauseMillis`
   - D) 分代 ZGC
...
>
> 4-B。G1 把堆切成等大的 Region(默认按「约 2048 个」反推单格尺寸,并向上取整到 2 的幂,范围 1MB–32MB;也可用 `-XX:G1HeapRegionSize` 直接指定),每个 Region 可动态扮演 Eden/Survivor/Old/Humongous。★举一反三:Region 化是 G1 实现「可预测停顿」的基础——只挑垃圾最多的 Region 收。
>
...
>
> 10-C。8G 在 G1 的舒适区内,默认 G1 + 合理的 `MaxGCPauseMillis` 即可;只有 P99 按毫秒考核(如高频交易)才值得上分代 ZGC(D)。★举一反三:不要过度设计——不是所有服务都需要 ZGC,很多 8G 堆的在线服务 G1 调优就够了。
>
> **解答题**
>
