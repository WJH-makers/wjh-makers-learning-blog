...
7. `chown user:group file.txt` 中冒号后面的 `group` 可以省略吗?省略后行为如何?
   - A) 不能省略,语法严格要求　B) 写成 `chown user. file.txt`(点号)时只改属主、不动组　C) 可以完全不写分隔符:`chown user file.txt` 只改属主、组保持不动;而 `chown user: file.txt`(冒号后为空)会把组改成该用户的主要组　D) 省略后组自动改为 root
