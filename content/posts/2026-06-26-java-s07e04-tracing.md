...
![《从零开始学 Java》55 · 链路追踪——阿零与豆豆六格漫画](/comics/java/s07e04-tracing.png)
...
1. TraceId、SpanId、parentSpanId 各自的作用是?
   - A) 三者等价　B) **TraceId 认「同一笔请求」(全链路唯一);SpanId 标「哪一跳」;parentSpanId 记「谁调用了我」**　C) TraceId 标方法名　D) SpanId 是时间戳
2. 散落各服务的 Span 靠什么重新拼成一棵调用树?
   - A) 时间戳排序　B) **parentSpanId 的父子关系**　C) 服务名　D) 日志顺序
3. TraceId 跨服务传播,现在的事实标准载体是?
   - A) URL 参数　B) Cookie　C) **W3C 的 `traceparent` HTTP 请求头**　D) 请求体字段
4. 把库存查询丢进裸 `new Thread` 后,链路断了,根因是?
   - A) 线程池满了　B) **追踪上下文绑在当前线程上(ThreadLocal/Context),裸线程不会自动继承**,出站请求没带 traceparent　C) 网络丢包　D) 采样率太低
5. 上述断链在下游表现为?
   - A) 请求失败　B) 下游收到没有通行证的请求,**另发一个全新的孤立 TraceId**,和主链对不上　C) 下游拒绝服务　D) Span 重复
6. 除了追踪上下文,裸 `new Thread` 还会丢掉什么?
   - A) 什么都不丢　B) **Spring Security 的 `SecurityContext`(当前用户)、MDC 日志上下文**　C) 数据库连接　D) 类加载器
7. 正确的异步写法是?
   - A) 继续用 `new Thread`,手动传参　B) 用被框架管理、会传播上下文的线程池(如注入的 `Executor`)　C) 改成同步　D) 用静态变量存上下文
8. 生产环境采样率通常设 0.1 而非 1.0,原因是?
   - A) 精度要求低　B) 全采样成本高,会把追踪系统自己压垮　C) 采样率越低越准　D) 标准规定
9. 关于采样,必须保证的一条是?
   - A) 每个服务独立决定采不采　B) **同一条 trace 要么全采、要么全不采**,否则链路会残缺　C) 只采样慢请求　D) 采样率必须整除
10. SkyWalking 的 Java Agent 埋点方式属于?
    - A) SDK 手动埋点　B) **字节码增强(无侵入)** —— 启动加 `-javaagent` 自动织入常见框架　C) 编译期插桩　D) 代理服务器抓包

> [!答案]
> **1-B**　三个 ID 各司其职。**举一反三**:只有 TraceId 没有 parent,你只能知道「这些日志属于同一次请求」,画不出调用树。
> **2-B**　父子关系还原树形。**举一反三**:所以任何一跳丢了 parent,那一整棵子树就从瀑布图上消失。
> **3-C**　`traceparent` 是 W3C Trace Context 标准。**举一反三**:MQ 场景则把它塞进消息头,原理完全一样。
> **4-B**　上下文跟着线程走,你换线程它不跟。**举一反三**:这是异步 + 追踪的头号坑,也是所有「ThreadLocal 型上下文」的通病。
> **5-B**　下游只好当成新请求。**举一反三**:看到 Jaeger 里出现大量「只有一跳的孤立 trace」,基本可以确定是某处上下文断了。
> **6-B**　安全上下文和日志 MDC 一起丢。**举一反三**:异步里取当前用户取到 null,十有八九就是这个原因 —— 这比链路断更容易引发线上事故。
> **7-B**　用被框架包装过的执行器。**举一反三**:这条纪律和第 41 话「别裸 new Thread,用线程池」是同一条,只是这次的理由从资源变成了上下文。
> **8-B**　追踪数据量极大。**举一反三**:所以生产常配「基础采样 + 错误/慢请求强制采样」,兼顾成本与排障需要。
> **9-B**　采样决策必须在入口做一次并随链路传播。**举一反三**:这也是为什么 `traceparent` 头里带有采样标志位 —— 决策要跟着走。
> **10-B**　Agent 无侵入。**举一反三**:代价是只能覆盖它认识的框架,自定义业务逻辑仍需 SDK 手动补 Span —— 两者常混着用。
