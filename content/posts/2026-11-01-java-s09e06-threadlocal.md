...
10. 关于 JDK 25 中 `ScopedValue` 相比 `ThreadLocal` 的优势,以下哪项描述最准确?
- A) `ScopedValue` 只是 `ThreadLocal` 的语法糖　　B) `ScopedValue` 是不可变的、有作用域的——绑定限定在 `where(...).run(...)` 的代码块内，适合将调用方的只读上下文单向传给下游　　C) `ScopedValue` 比 `ThreadLocal` 更快,因为使用 CAS 实现　　D) `ScopedValue` 会自动传播到任意线程池或 CompletableFuture 任务
...
>
> **1-10** B(ScopedValue 的优势是不可变绑定、受作用域约束，并可在结构化并发的子任务中高效继承。它不随虚拟线程的 mount/unmount「传播」，也不会自动跨任意 Executor/CompletableFuture 传递；是否选它取决于数据是否是不可变、单向上下文。)
...
>
> **2-4** 根因:`InheritableThreadLocal` 的拷贝只在 `new Thread()` 时发生——JVM 在 `Thread.init()` 中拷贝父线程的 `inheritableThreadLocals`,之后父线程的修改不会同步到已创建的子线程。线程池的核心线程在池启动时就创建好了,任务提交后只复用这些已存在的线程,不会再走 init 路径。解决方案:① 线程池场景下不依赖 `InheritableThreadLocal`,可在提交边界显式传递上下文，或在明确接受其依赖和语义后使用 TTL。② JDK 25 的 ScopedValue 适合结构化并发中的不可变上下文，会传给该 scope fork 的子任务；它不是平台线程池的通用自动传播器，且 StructuredTaskScope 仍是预览 API。
...
>
> **2-5** 设计决策:① 用 `ScopedValue` 而非 `ThreadLocal`——需求中的上下文是请求级别的不可变信息(traceId/userId/tenantId),天然适配 ScopedValue 的不可变和作用域自动清理。② 异步传递:在结构化并发(`StructuredTaskScope`)中 fork 的子任务会继承绑定；如果用 CompletableFuture/普通 Executor,它**不会自动继承**，应显式传参或在提交的 lambda 中重新 `where(...).run(...)` 绑定。③ 局部覆盖:ScopedValue 不可变,不能 `set`。解决方案——在需要覆盖 userId 的地方,对同一个 key 建立更内层的绑定，退出内层作用域后自动恢复外层值。
> ```java
> // 定义三个 ScopedValue
...
>               try (var scope = StructuredTaskScope.open()) { // JDK 25 预览 API；编译和运行均需 --enable-preview
