...
>
> **2-1** 锁的是「obj」这个**对象**,不是代码块。`synchronized(obj)` 执行时,JVM 尝试获取 obj 对象头 Mark Word 中的锁记录。无锁状态下 Mark Word 存 hashCode 和 GC 分代年龄;进入 `synchronized` 后,Mark Word 被替换为指向线程栈中 Lock Record 的指针(轻量级锁),或者指向 monitor 对象的指针(重量级锁)。解锁时再将 Mark Word 恢复。整个过程中,被保护的代码块只是关联到锁对象上,锁的拥有权和释放都只跟 obj 有关。所以两个线程对**同一个对象**加锁才会互斥;对两个不同对象加锁,即使执行的代码完全相同,也互不影响。  
> **举一反三**:这是「synchronized 是可重入的」的基础——同一线程再次进入同一锁时,检查 Mark Word 就会发现锁记录指向自己线程,不需要竞争。可重入靠的就是 Mark Word 里记录「当前持有者线程」。
>
> **2-2** JDK 17+ 路径图:
> ```
...
>                              重量级锁(OS mutex)
> ```
> 触发升级:轻量级→重量级:① CAS 自旋超过阈值(-XX:PreBlockSpin 默认 10 次) ② 锁被持有时间过长 ③ 调用了 `wait()`(wait 必须配合重量级 monitor)。降级:锁只升不降——但锁被释放后会回到无锁状态(撤销重量级锁),下次需要时重新走升级路径。偏向锁移除原因:偏向锁的设计假设「同一个线程反复获取和释放锁」,通过偏向模式避免 CAS。但现代高并发应用多个线程交替竞争的场景远多于单线程独占,偏向锁的撤销(需要在全局安全点 STW 执行)成本反而比省掉的 CAS 还大,得不偿失。  
> **举一反三**:锁升级是单向的是因为降级判断的复杂度和代价超过收益——JVM 要在「该不该降」这个问题上付出的检查成本,可能比直接用轻量级锁重来一遍更贵。所以选择了简单粗暴的「只升不降,靠释放回到无锁重新来过」。
>
