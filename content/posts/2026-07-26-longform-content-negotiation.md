...
> [!警告]
> **本文描述的功能已于 2026 年 8 月下线,现在照着试会拿到 404。**
> `/llms.txt`、`/posts/<slug>/markdown` 与 `Accept: text/markdown` 协商都已从本站移除,
> 连生成器 `lib/agent-markdown.ts` 一起删掉了 —— 只守出口不删生成器的话,它会作为
> "已导出但无调用方"的死代码留着,下一个人很容易读成漏接线的 bug 而去把路由补回来。
> 移除的原因是不再对外提供全文批量导出;仓库里有两条测试钉住这个决定,防止日后又被加回来。
> 下面的工程内容(`Vary: Accept` 的缓存债、Edge Runtime 不能用 `node:crypto`、
> 共享缓存投毒的成因)本身没有过期,当技术笔记读仍然成立 —— 只是别把它当本站的现役接口文档。
...
> [!NOTE]
> **Next.js 16 起该约定已改名**:文件 `middleware.ts` → `proxy.ts`,导出 `middleware` → `proxy`,能力完全不变(官方 codemod:`npx @next/codemod@latest middleware-to-proxy .`)。本文行文沿用 middleware 这个通行叫法,本站仓库里的实际文件已是 `proxy.ts`。
