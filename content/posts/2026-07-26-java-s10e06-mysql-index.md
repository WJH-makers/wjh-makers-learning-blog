...
9. 阿零执行 `SELECT * FROM member WHERE level=3 AND name LIKE '张%'`,索引是 `(level, name)`。关于索引使用,描述正确的是?
   - A) 只能用到 level,范围条件本身不算命中
   - B) level 和 name 都用到了(等值定位第一段 + 前缀 LIKE 在第二段走 range)
   - C) 完全用不到索引
   - D) 需要回表才能拿到 name
...
>
> 2-B。聚簇索引的叶子节点直接存放整行数据(InnoDB 的表即索引组织表)。★举一反三:InnoDB 一定有聚簇索引,但未必是你定义的主键——挑选顺序是①用户主键 ②第一个「所有列都 NOT NULL」的唯一索引 ③都没有,才生成 6 字节隐藏 row_id。中间那一步最常被漏掉。
>
...
>
> 9-B。`level=3` 等值命中第一段;`name LIKE '张%'` 是前缀匹配,优化器把它转成范围条件,在第二段走 range——**范围列本身是用到的**,停用的是它后面的列(对照第五节的最左前缀表:`a=1 AND b>2 AND c=3` 用到 a、b,断在 c)。`key_len` 可确证:`level`(TINYINT NOT NULL)1 + `name`(VARCHAR(50) utf8mb4)50×4+2=202,合计 **203**;若只命中 level 则是 1。★举一反三:「LIKE 能不能走索引」和「走到第几段」是两个问题——前者看有没有前导 `%`,后者看最左前缀断在哪。
>
