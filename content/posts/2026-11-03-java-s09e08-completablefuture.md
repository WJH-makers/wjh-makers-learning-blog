...
> **〔2〕** JVM 城主推开线程调度中心的侧门:一条公共流水线,牌匾写着 **ForkJoinPool.commonPool**。默认配置下它通常按「CPU 核数减一」设并行度,但也能被 JVM 参数改掉。
...
不带 Async 的 `thenApply` 由**完成上一步的线程顺手执行**——这里就是 commonPool 工人。公共池的并行度有限且可配置,一人一通两秒长途就足以把它拖住。
...
同样姿势的 worker 数量由公共池配置决定——**全体举着电话**。commonPool 是全 JVM 共享的计算池,适合短平快的 CPU 活;被阻塞 IO 占满后,所有借用它的人——并行流(回看第 68 话)、别人家的 CompletableFuture——一起陪葬。这不是慢,是**店级传染**。

> **🎯 面试直击**:thenApply 和 thenCompose 什么区别?默认线程池是谁、坑在哪?
> thenApply 接「值→值」,若下一步本身返回 CompletableFuture 会得到套娃;thenCompose 接「值→票据」并拍平——正是 map 与 flatMap 的关系。未指定 Executor 的异步方法默认使用 ForkJoinPool.commonPool;它全 JVM 共享、并行度有限且可配置,适合 CPU 短任务,塞进阻塞 IO 会占满工人,连并行流一起拖慢。追问点:IO 密集怎么办?——用 *Async 重载传自定义 Executor,快慢活分池。
...
- A) 一个专门的 `CompletableFuture` 线程池　　B) `ForkJoinPool.commonPool()`——JDK 的公共 ForkJoinPool,默认并行度常为 CPU 核数 - 1,但可配置　　C) `Executors.newCachedThreadPool()`　　D) 主线程
...
>
> **1-5** B(ForkJoinPool.commonPool()——未传 Executor 的异步方法默认使用它。常见默认并行度约为 `availableProcessors() - 1`,但可由 `java.util.concurrent.ForkJoinPool.common.parallelism` 覆盖;排障时用 `ForkJoinPool.getCommonPoolParallelism()` 看实际值。)
...
>
> **1-6** B(阻塞 IO 占用公共 ForkJoinPool → 线程饥饿——有限的公共池 worker 被阻塞 IO 卡住,其他使用公共池的异步任务(如 parallelStream、其他 CompletableFuture)会受影响。修法:`supplyAsync(heavyIO, ioExecutor)` 传入按容量治理的专用 Executor,不要把无界 CachedThreadPool 当成默认处方。)
...
>
> **1-10** B(虚拟线程让「同步风格写异步代码」成为现实——阻塞 IO 时虚拟线程会从 carrier 卸载,不需要回调。但 CPU 密集型的任务编排仍需合适的并行模型;`StructuredTaskScope` 可让任务生命周期更清晰,不过截至 JDK 25 仍是预览 API。)
> **举一反三**:Java 21+ 可把 IO 密集任务写成虚拟线程上的同步代码;CPU 密集编排则显式选择合适的 Executor/ForkJoinPool。需要结构化并发时,Java 25 要加 `--enable-preview` 使用 `StructuredTaskScope`。
>
> **2-1** Future 写法:
> ```java
...
>
> **2-5** ① 阻塞 IO 场景:虚拟线程会从 carrier(平台线程)卸载,让 carrier 去运行其他虚拟线程;这降低了「阻塞等待占住 OS 线程」的成本，但不会替下游数据库、连接数或限流兜底。`Thread.ofVirtual().start(() -> blockingIO())` 使用 JDK 的虚拟线程调度器，不是把任务塞进任意线程池；公共 ForkJoinPool 里仍不该跑阻塞 IO。② CPU 密集型编排:虚拟线程**不能替代**——它不增加 CPU 核数。CPU 密集的并行计算应使用按核数和任务特点配置的 Executor/ForkJoinPool。③ 异常处理:同步风格的 try-catch 在虚拟线程中同样有效;是否保留 `CompletableFuture` 取决于是否需要它的组合 API。既有 IO 又有 CPU 时，可在 Java 25 的预览 `StructuredTaskScope` 中组织 IO 子任务，再做 CPU 合并。
