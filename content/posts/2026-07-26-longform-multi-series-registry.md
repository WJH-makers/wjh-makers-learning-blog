...
> [!NOTE]
> 文中三处把 `llms.txt` 列为注册表的下游出口(第 1、6 节)。**这个出口已于 2026 年 8 月移除**,
> 现在 `/llms.txt` 返回 404 —— 原因与注册表无关,是不再对外提供全文批量导出。
> 注册表本身以及 sitemap / footer / OG 那几个出口都还在,本文的论证不受影响,
> 只是「自动进 llms.txt」这一项今天已经不成立。
