...
1. 业务异常大多继承 `RuntimeException` 而不是 `Exception`,主要理由是?
   - A) 运行更快　B) 否则从 `order()` 到 `main()` 每层都得写 `throws`,接口被污染　C) 语法更简单　D) 可以携带字段
2. 自定义异常忘了调 `super(message)`,后果是?
   - A) 编译报错　B) `getMessage()` 返回 `null`,栈轨迹里那句人话消失　C) 异常无法被抛出　D) 类型信息丢失
3. 异常的消息实际存在哪里?
   - A) 子类的字段里　B) 父类 `Throwable` 里,只有 `super(message)` 能写进去　C) 线程局部变量　D) JVM 的常量池
4. 「异常链 cause」的作用是?
   - A) 让异常更短　B) 转译异常时保留根本原因,栈轨迹里出现 `Caused by:`　C) 自动重试　D) 合并多个异常
5. 在 `catch (IOException e)` 里转译成业务异常,却没把 `e` 当 cause 传进去,后果是?
   - A) 编译报错　B) 案发现场只剩一句结论、没有监控录像 —— 原始栈丢失　C) 异常无法被捕获　D) 消息变成 null
6. 给异常带上 `item` / `requested` / `available` 字段的价值是?
   - A) 让异常更大　B) 上层能精准响应(缺货提示补单、无此单品推荐替代)　C) 提高性能　D) 便于序列化
7. `assertThrows` 的返回值是?
   - A) `boolean`　B) 捕获到的异常对象,可继续断言它的字段　C) `void`　D) 异常消息字符串
8. 什么时候才该让自定义异常继承 `Exception`(checked)?
   - A) 任何业务错误　B) 想**强制**调用方处理的、可恢复的错误　C) 需要携带字段时　D) 需要被日志记录时
9. 异常链是哪个版本引入的?
   - A) Java 1.2　B) Java 1.4　C) Java 5　D) Java 7
10. 美式库存 1 杯、顾客要 5 杯,`e.shortfall()` 应该是?
    - A) `5`　B) `1`　C) `4`　D) `6`

> [!答案]
> **1-B**　unchecked 保持调用链干净。**举一反三**:代价是调用方可能忘了处理,所以要配统一异常处理器兜底(第四季讲)。
> **2-B**　祖宗那半张嘴没接上,铭牌一片空白。**举一反三**:排障时看到「异常类型对、消息为 null」,先去看构造器有没有调 `super`。
> **3-B**　消息是 `Throwable` 的字段。**举一反三**:同理 `cause` 也在父类,所以要支持异常链就得再加一个 `(String, Throwable)` 构造器。
> **4-B**　它把「结论」和「现场」串起来。**举一反三**:读栈轨迹时 `Caused by` 往下才是根因,最上面那个往往只是包装层。
> **5-B**　丢了 cause 等于毁掉证据。**举一反三**:这也是为什么 `catch` 里只打印 `e.getMessage()` 不够 —— 要打完整栈。
> **6-B**　异常本身就是一份可编程的数据。**举一反三**:这也是「异常不只是报错,而是带上下文的信号」的含义。
> **7-B**　它把异常对象还给你。**举一反三**:所以能进一步断言业务字段,测试才算真正钉死了行为而不只是「抛了个错」。
> **8-B**　checked 的意义就是「编译器逼你面对」。**举一反三**:`IOException` 就是典型 —— 文件可能不存在,是调用方有理由预期并处理的外部意外。
> **9-B**　Java 1.4 引入异常链。**举一反三**:在那之前转译异常会直接丢现场,老代码里的「无头异常」很多源于此。
> **10-C**　`requested - available` = 5 - 1 = 4。**举一反三**:把这种计算封进异常的方法里,比让每个调用方各算一遍更不容易出错。
